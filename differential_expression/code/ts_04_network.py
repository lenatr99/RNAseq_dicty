...
DATA_PATH = f"../data/time_series/communities/{community_mode}/"
...
def save_plot(event):
    ...
    plt.draw()
    name = f"{community_mode}_{TERM}_{strain}.svg"
    plt.savefig(
...
communities_sets = load_data("json/communities.json")

for i in range(len(SETS)):
    GENE_SET, TERM = SETS[i], TERMS[i]
    graph = all_graph_data[(strain, GENE_SET, TERM)]
    communities = all_communities[(strain, GENE_SET, TERM)]
    graph_cc = all_graph_data_cc[(strain, GENE_SET, TERM)]
    communities_cc = communities_sets[strain][TERM]
    graph_layout = all_graph_layout[(strain, GENE_SET, TERM)]

    com_names_lgg = communities_desc[strain][TERM]
    ...
    k = 0
    for com in communities_cc.values():
        if k >= 4:
            break
    ...
    for i in range(len(com_names_lgg)):
        community_nodes = communities_cc[str(i)]
    ...
    print(
        f"Now looking at gene set {GENE_SET} for strain {strain} and term {TERM}. Move the community descriptions around to make the plot more readable and click 'Save Plot' to save the plot."
    )
