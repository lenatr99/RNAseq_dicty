...
for scaling in SCALING:
    ...
    for i, strains_sub in enumerate(split_strains):
        ...
        all_interpolated_curves = [
            interpolate_curve(
                (
                    list(mappings[scaling][strain].values()),
                    list(mappings[scaling][strain].keys()),
                ),
                times_fine,
            )
        ...
        for j, strain in enumerate(strains_sub):
            sorted_X, sorted_Y = list(mappings[scaling][strain].values()), list(
                mappings[scaling][strain].keys()
            )
